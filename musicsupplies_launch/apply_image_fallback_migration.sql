...
-- This creates the image fallback function for the product image system
...
BEGIN
    ...
    IF hyphen_pos1 > 0 THEN
        ...
        IF hyphen_pos2 > 0 THEN
            -- Get base partnumber (first two parts: "GUITAR-ACOUSTIC" from "GUITAR-ACOUSTIC-STEEL")
        ...
        -- If no match with 2 parts, try matching just the first part ("GUITAR" from "GUITAR-ACOUSTIC-STEEL")
        SELECT COALESCE(p.groupedimage, r.image_name)
        INTO fallback_image
        FROM products_supabase p
        LEFT JOIN rt_extended r ON p.partnumber = r.part_number
...
-- SELECT get_fallback_image_for_product('TEST-PRODUCT-123', 'TestBrand', 'TestCategory', 'TestSubCategory');
