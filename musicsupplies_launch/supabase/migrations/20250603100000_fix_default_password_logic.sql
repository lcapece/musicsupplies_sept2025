/*
  # Fix default password logic to use full zip code
  
  Update the authenticate_user_lcmd function to:
  1. Use the full zip code (not just first 5 digits)
  2. Ensure case-insensitive comparison for default passwords only
  
  Default password format: First letter of account name + full zip code (case insensitive)
*/

-- Update the authentication function to use full zip code for default password
...
-- Add comment to document the password logic
...
2. Default password: first letter of account name + full zip code (case insensitive)
