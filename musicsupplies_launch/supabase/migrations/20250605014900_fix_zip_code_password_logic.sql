/*
  # Fix ZIP code password logic to use only first 5 digits
  
  The test accounts documentation clearly states that default passwords should use 
  only the first 5 digits of ZIP codes, but the current function uses the full ZIP.
  
  This migration fixes that discrepancy.
*/

-- Update the authentication function to use only first 5 digits of ZIP code for default password
...
BEGIN
  ...
  -- Default pattern: first letter of account name + FIRST 5 DIGITS OF ZIP CODE (case insensitive for default passwords only)
  IF account_record.acct_name IS NOT NULL AND account_record.zip IS NOT NULL THEN
    -- Extract first 5 digits of ZIP code (remove any dashes or extensions)
    zip_first_five := substring(account_record.zip from 1 for 5);
    
    -- Create expected default password: first letter of account name + first 5 digits of zip code
    expected_default_password := lower(substring(account_record.acct_name from 1 for 1)) || lower(zip_first_five);
...
-- Add comment to document the corrected password logic
COMMENT ON FUNCTION authenticate_user_lcmd(bigint, text) IS 
'Authenticates users with either:
1. Hashed password from logon_lcmd table (case sensitive)
2. Default password: first letter of account name + FIRST 5 DIGITS OF ZIP CODE (case insensitive)
Default passwords require password change on first login.
IMPORTANT: Uses only first 5 digits of ZIP code, not full ZIP code with extensions.';
