-- FINAL FIX WITH CORRECT COLUMN NAMES
...
BEGIN
    ...
    -- Check if account exists - USING ACTUAL COLUMN NAMES
    SELECT 
        COUNT(*) > 0,
        MAX(jsonb_build_object(
            'account_number', account_number,
            'acct_name', acct_name,
            'contact', contact,  -- FIXED: was contact_name
            'email', email_address,  -- FIXED: was email
            'phone', phone,
            'zip', zip,
            'address', address,
            'city', city,
    ...
    FROM accounts_lcmd
    WHERE account_number::text = p_account_number;  -- Cast integer to text for comparison
