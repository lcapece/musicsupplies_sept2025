...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // First check if this is account 999 - use security questions instead of 2FA
      if (identifier === '999') {
        const { data, error } = await supabase.rpc('authenticate_user_with_security_question', {
          p_identifier: identifier,
          p_password: password
        });
        
        if (error) {
          console.error('Security question auth error:', error);
          setIsLoading(false);
          return;
        }
        ...
        if (data && data.length > 0) {
          const result = data[0];
          if (result.requires_security_question && result.security_question_id) {
            // Show security question
            setSecurityQuestionId(result.security_question_id);
            setSecurityQuestionText(result.security_question_text);
            setShowSecurityQuestion(true);
            setIsLoading(false);
            return;
          } else if (result.success) {
            // Login successful - let AuthContext handle the rest
  ...
  const handleSecurityQuestionSubmit = async (e: React.FormEvent) => {
    ...
    if (!securityAnswer.trim() || !securityQuestionId) {
      return;
    }
    ...
    try {
      const { data, error } = await supabase.rpc('authenticate_user_with_security_question', {
        p_identifier: identifier,
        p_password: password,
        p_question_id: securityQuestionId,
      ...
      if (data && data.length > 0) {
        const result = data[0];
        if (result.success) {
          // Security question passed - complete login
          setShowSecurityQuestion(false);
          setSecurityAnswer('');
          // Let AuthContext handle the final login
          await login(identifier, password);
        }
      }
    } catch (err) {
      console.error("Security question submit error", err);
    } finally {
      setIsSecurityLoading(false);
    }
  };
  ...
  const renderForm = () => {
    if (showSecurityQuestion) {
      ...
      return (
        <form onSubmit={handleSecurityQuestionSubmit} className="w-full max-w-[clamp(20rem,35vw,28rem)]">
          <div className="bg-green-50 border border-green-200 text-green-700 px-[1.5vw] py-[2vh] rounded mb-[2vh] text-[clamp(0.875rem,1.2vw,1rem)]">
            <div className="font-semibold mb-1">Security Question Required</div>
            <div>Please answer the following security question to complete your login:</div>
          </div>
          {error && (
          ...
          <div className="mb-[2vh]">
            <label className="block text-[clamp(0.875rem,1.2vw,1rem)] font-medium text-gray-700 mb-[1vh]">
              Security Question:
            </label>
            <div className="bg-gray-50 border border-gray-200 p-[1.5vw] rounded text-[clamp(0.875rem,1.2vw,1rem)] mb-[1vh]">
              {securityQuestionText}
            </div>
          </div>
          <div className="mb-[2vh]">
            <label htmlFor="securityAnswer" className="block text-[clamp(0.875rem,1.2vw,1rem)] font-medium text-gray-700 mb-[0.5vh]">
              Your Answer
            </label>
            <div className="relative">
              ...
              <input
                type="text"
                id="securityAnswer"
                className="w-full pl-[3vw] pr-[1vw] py-[1.2vh] text-[clamp(1rem,1.4vw,1.125rem)] border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
          ...
          <button
            type="submit"
            className="w-full bg-green-600 text-white py-[1.5vh] px-[2vw] text-[clamp(1rem,1.4vw,1.125rem)] font-medium rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 transition-colors duration-200"
            disabled={isSecurityLoading || !securityAnswer.trim()}
          >
            {isSecurityLoading ? 'Verifying Answer...' : 'Submit Answer'}
          </button>
