...
-- PROPERLY DEPLOYED TO HOSTED SUPABASE
...
BEGIN
  ...
  -- Check if account has custom password in user_passwords table ONLY
  SELECT password_hash INTO v_password_hash
  FROM user_passwords 
  WHERE account_number = v_account_record.account_number;

  IF v_password_hash IS NOT NULL THEN
    -- Has custom password - verify using crypt()
    IF crypt(p_password, v_password_hash) = v_password_hash THEN
      RETURN QUERY SELECT
        ...
        false::boolean, -- requires_password_change
        (v_account_record.account_number = 99)::boolean, -- is_special_admin
        false::boolean, -- needs_password_initialization
        false::boolean  -- requires_2fa
      ;
    END IF;
  ELSE
    -- No custom password - check ZIP code authentication
    IF v_account_record.acct_name IS NOT NULL AND v_account_record.zip IS NOT NULL THEN
      ...
      IF LOWER(TRIM(p_password)) = v_zip_password THEN
        ...
        RETURN QUERY SELECT
          ...
          false::boolean, -- requires_password_change
          (v_account_record.account_number = 99)::boolean, -- is_special_admin
          true::boolean,  -- needs_password_initialization (CRITICAL!)
          false::boolean  -- requires_2fa
        ;
      END IF;
    END IF;
