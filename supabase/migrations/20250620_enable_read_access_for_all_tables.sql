...
-- Helper function to create read-only policies for a table
...
EXCEPTION
    ...
    WHEN undefined_object THEN
        RAISE NOTICE 'Cannot modify % (might be a view), skipping', table_name;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    PERFORM create_read_policy_for_table('treeview_datasource');
    ...
    FOR tbl IN 
        ...
        AND tablename NOT IN (
            ...
            'treeview_datasource', 'accounts_lcmd', 'logon_lcmd', 'ordhist', 
...
-- Handle views separately (they don't support RLS but need permissions)
DO $$
DECLARE
    view_rec RECORD;
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE 'Granting read permissions on views...';
    
    -- Grant permissions on known views
    GRANT SELECT ON products_supabase TO anon, authenticated;
    RAISE NOTICE 'Granted read access on view: products_supabase';
    
    -- Grant permissions on any other views
    FOR view_rec IN 
        SELECT viewname 
        FROM pg_views 
        WHERE schemaname = 'public'
    ...
    LOOP
        ...
        RAISE NOTICE 'Granted read access on view: %', view_rec.viewname;
    END LOOP;
END $$;

-- Clean up the helper function
DROP FUNCTION create_read_policy_for_table(text);
