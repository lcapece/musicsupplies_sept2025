...
-- Ensures ORDER_ITEMS is always stored as a single valid JSON array

-- Create a function to validate and fix JSON array formatting
...
-- Create trigger to validate ORDER_ITEMS before insert or update
...
-- Update the complete_order function to ensure proper JSON array formatting
...
-- Add a constraint to ensure ORDER_ITEMS is always a valid JSON array (if not null)
...
-- Create a function to fix existing malformed ORDER_ITEMS data
...
-- Run the fix for any existing malformed data
-- (Comment this out after first run if you don't want it to run on every migration)
